...
def timer_start(label: str, end_ts: int) -> discord.Embed:
    ...
    return create_embed("Time's Ticking!", desc, colour_key="utility")


def timer_finished(label: str, end_ts: int) -> discord.Embed:
    desc = f"**{label}** is done!\nEnded at: <t:{end_ts}:t>"
    return create_embed("Ding ding!", desc, colour_key="success")
...
def choose_result(choice: str) -> discord.Embed:
    return create_embed("I choose...", f"**{choice}**", colour_key="fun")
...
def flip_result(result: str) -> discord.Embed:
    return create_embed("Coin Flip", f"It's **{result}**!", colour_key="utility")


def flip_guess(result: str, guess: str, won: bool) -> discord.Embed:
    desc = f"Guess: **{guess}**\nResult: **{result}**\n\n"
    desc += "✨ Correct!" if won else "❌ Wrong."
...
def rate_thing(thing: str, score: int) -> discord.Embed:
    ...
    if score == 10:
        embed.set_footer(text="Absolute perfection.")
    return embed
